...
public class plus_plus {
    ...
    /* script pour le test de plus_plus.deca */
    public static AbstractProgram ProgInit() {
        ...
        AbstractExpr left = new IntLiteral(5);
        AbstractExpr middle = new IntLiteral(2);
        AbstractExpr right = new IntLiteral(3);
        AbstractExpr tmp = new Plus(left, middle);
        AbstractExpr expr = new Plus(tmp, right);
        linst.add(expr);
    ...
    public static void genSyntaxTreeParserFile(String[] sourceName) throws IOException {
        ...
        if (prog == null) {
        	...
        	PrintStream ps = new PrintStream(new FileOutputStream(currentUsersDir + "/obtained/plus_plus.txt" , true));
            prog.prettyPrint(ps);
            ps.close();
        }
    }
    ...
    public static void genSyntaxTreeManualFile(AbstractProgram source) throws IOException {        
        PrintStream ps = new PrintStream(new FileOutputStream(currentUsersDir + "/expected/plus_plus.txt", true));
    ...
    public static void main(String args[]) throws IOException {
    	...
    	String path = currentUsersDir.substring(0, userspathlength - 41) + "/deca/syntax/valid/created/plus_plus.deca";
