...
public class unary {
	...
	/* script pour le test de unary.deca */
    public static AbstractProgram ProgInit() {
        ...
        AbstractExpr expr = new UnaryMinus(new IntLiteral(1));
        linst.add(expr);
        AbstractExpr expr2 = new FloatLiteral((float) 8.7);
        linst.add(new Assign(new Identifier(compiler.getSymbolTable().create("a")), new Not(expr2)));
    ...
    public static void genSyntaxTreeParserFile(String[] sourceName) throws IOException {
        ...
        if (prog == null) {
        	...
        	PrintStream ps = new PrintStream(new FileOutputStream(currentUsersDir + "/obtained/unary.txt" , true));
            prog.prettyPrint(ps);
            ps.close();
        }
    }
    ...
    public static void genSyntaxTreeManualFile(AbstractProgram source) throws IOException {        
        PrintStream ps = new PrintStream(new FileOutputStream(currentUsersDir + "/expected/unary.txt", true));
    ...
    public static void main(String args[]) throws IOException {
    	...
    	String path = currentUsersDir.substring(0, userspathlength - 41) + "/deca/syntax/valid/created/unary.deca";
