...
public class printlnx {
	...
	/* script pour le test de printlnx.deca */
    public static AbstractProgram ProgInit() {
        ...
        lexpr.add(new FloatLiteral((float) 5.2256));
		linst.add(new Println(true, lexpr));
    ...
    public static void genSyntaxTreeParserFile(String[] sourceName) throws IOException {
        ...
        if (prog == null) {
        	...
        	PrintStream ps = new PrintStream(new FileOutputStream(currentUsersDir + "/obtained/printlnx.txt" , true));
            prog.prettyPrint(ps);
            ps.close();
        }
    }
    ...
    public static void genSyntaxTreeManualFile(AbstractProgram source) throws IOException {        
        PrintStream ps = new PrintStream(new FileOutputStream(currentUsersDir + "/expected/printlnx.txt", true));
    ...
    public static void main(String args[]) throws IOException {
    	...
    	String path = currentUsersDir.substring(0, userspathlength - 41) + "/deca/syntax/valid/created/printlnx.deca";
