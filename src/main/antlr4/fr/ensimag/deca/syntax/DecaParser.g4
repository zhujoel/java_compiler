...
inst returns[AbstractInst tree]
    ...
    | SEMI {
    		// ça retourne ';'...
    		$tree = new StringLiteral($SEMI.text);
        }
...
@init {
    ...
    : if1=IF OPARENT condition=expr CPARENT OBRACE li_if=list_inst CBRACE {
    		// ceci est tres probablement faux
...
mult_expr returns[AbstractExpr tree]
    ...
    | e1=mult_expr TIMES e2=unary_expr {
            assert($e1.tree != null);                                         
            assert($e2.tree != null);
            $tree = new Times($e1.tree, $e2.tree);
        }
    | e1=mult_expr SLASH e2=unary_expr {
            assert($e1.tree != null);                                         
            assert($e2.tree != null);
            $tree = new Slash($e1.tree, $e2.tree);
        }
    | e1=mult_expr PERCENT e2=unary_expr {
            assert($e1.tree != null);                                                             
            assert($e2.tree != null);
            $tree = new Percent($e1.tree, $e2.tree);
        }
    ;
...
ident returns[AbstractIdentifier tree]
    : IDENT {
    		// idk
    		$tree = new Identifier(Symbol.create($IDENT.text));
        }
    ;
